...
import sys
import uuid
from typing import Any, TypeVar, Type, Union, Dict, Optional, cast, get_origin, get_args
...
def parse_json(text: str, target_type: Optional[Type[T]] = None) -> Union[T, Any]:
    ...
    # Strip control characters that are invalid in JSON strings (RFC 8259 §7)
    # Keeps \n (\x0a), \r (\x0d), \t (\x09) which are valid JSON whitespace
    json_str = re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f]', '', json_str)

    try:
        result = json.loads(json_str)
    except json.JSONDecodeError as e:
        # Last-ditch: repair truncated / malformed LLM JSON (unclosed strings,
        # missing delimiters, trailing prose). Keeps good data instead of
        # dropping a whole plan because of one syntax error.
        try:
            from json_repair import repair_json
        except ImportError:
            raise ValueError(f"Failed to parse JSON: {e}. Text was: {json_str[:200]}...")
        repaired = repair_json(json_str, return_objects=True)
        # repair_json returns "" for inputs it can't salvage — treat that as failure
        if repaired == "" or repaired is None:
            raise ValueError(f"Failed to parse JSON (repair also failed): {e}. Text was: {json_str[:200]}...")
        result = repaired

    # If target_type is provided and has from_dict/parse_obj/model_validate methods (Pydantic)
    if target_type and get_origin(target_type) is not None:
        # Handle List[SomeType] case
        if get_origin(target_type) is list:
            item_type = get_args(cast(Any, target_type))[0]
        ...
        _type_any = cast(Any, target_type)
        if hasattr(_type_any, 'model_validate'):
            result = _type_any.model_validate(result)
        elif hasattr(_type_any, 'parse_obj'):
            result = _type_any.parse_obj(result)
