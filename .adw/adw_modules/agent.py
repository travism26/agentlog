...
class AgentPromptRequest(BaseModel):
    ...
    mcp_config: Optional[str] = None  # Path to MCP config file (e.g., .mcp-browser.json)
...
# Hang-detection constants for claude -p subprocesses
_COMPLETION_GRACE = 30   # seconds after result message to wait for clean exit
_POLL_INTERVAL = 2       # seconds between polling checks
...
# Model mapping for slash commands: {command: {model_set: model_name}}
# "base" uses sonnet for most commands; "heavy" upgrades planning/implementation to opus
SLASH_COMMAND_MODEL_MAP: Dict[str, Dict[str, str]] = {
    ...
    "/validate": {"base": "sonnet", "heavy": "sonnet"},
    "/resolve_validation_violation": {"base": "sonnet", "heavy": "sonnet"},
...
def get_model_for_slash_command(request: "AgentTemplateRequest") -> str:
    """Determine the model to use for a slash command based on ADW state model_set.

    Loads the ADW state to get the model_set ("base" or "heavy"), then looks up
    the appropriate model for the slash command in SLASH_COMMAND_MODEL_MAP.
    Defaults to "sonnet" if state is missing or command is not in the map.

    Args:
        request: The AgentTemplateRequest containing adw_id and slash_command

    Returns:
        Model name string ("sonnet" or "opus")
    """
    ...
    if state:
        model_set = state.get("model_set", "base") or "base"

    # Look up the model for this command and model_set
...
def prompt_claude_code(request: AgentPromptRequest) -> AgentPromptResponse:
    ...
    # Check for MCP config - use specified config or fall back to .mcp.json in working dir
    if request.mcp_config:
        # Use explicitly specified MCP config
        if os.path.isabs(request.mcp_config):
            mcp_config_path = request.mcp_config
        else:
            # Relative path - resolve from working_dir or current dir
            base_dir = request.working_dir or os.getcwd()
            mcp_config_path = os.path.join(base_dir, request.mcp_config)
        if os.path.exists(mcp_config_path):
            cmd.extend(["--mcp-config", mcp_config_path])
    elif request.working_dir:
        # Fall back to default .mcp.json in working directory
    ...
    # Explicitly enable project and user skills
    cmd.extend(["--setting-sources", "project,user"])

    # Set up environment with only required variables
    # Note: Using None for env means inherit the parent process environment,
    # which includes Claude Code's authentication
    env = None  # Inherit parent environment to get Claude Code's auth

    logger.debug(
        "prompt_claude_code — agent=%s model=%s prompt_len=%d cwd=%s",
        request.agent_name, request.model, len(request.prompt), request.working_dir,
    )

    try:
        # Open output file for streaming via Popen so we can detect hangs.
        # claude -p sometimes writes its final result but never exits;
        # subprocess.run would block forever. Instead we poll the JSONL
        # for the result message and kill the process after a grace period.
        with open(request.output_file, "w") as output_f:
            ...
            # Poll for process exit, with hang detection
            result_found_at = None
            was_force_killed = False

            while proc.poll() is None:
                # Check if the JSONL already contains a result message
                if result_found_at is None:
                    try:
                        _, result_msg = parse_jsonl_output(request.output_file)
                        if result_msg:
                            result_found_at = time.time()
                            logger.info(
                                "Result message detected in JSONL — waiting %ds for clean exit (pid=%d)",
                                _COMPLETION_GRACE, proc.pid,
                            )
                    except Exception:
                        pass

                # Grace period expired — kill the hung process
                if result_found_at and (time.time() - result_found_at) > _COMPLETION_GRACE:
                    logger.warning(
                        "Claude process hung after writing result — killing (pid=%d, grace=%ds)",
                        proc.pid, _COMPLETION_GRACE,
                    )
                    proc.kill()
                    try:
                        proc.wait(timeout=5)
        ...
        # If we force-killed after a successful result, treat as returncode 0
        effective_returncode = 0 if was_force_killed else proc.returncode
        ...
        result = _ProcResult(effective_returncode, stderr_output)

        logger.debug("prompt_claude_code — returncode=%d output_file=%s", result.returncode, request.output_file)
...
def execute_template(request: AgentTemplateRequest) -> AgentPromptResponse:
    ...
    logger.debug(
        "execute_template — agent=%s cmd=%s model=%s output=%s",
        request.agent_name, request.slash_command, request.model, output_file,
    )
    ...
    prompt_request = AgentPromptRequest(
        ...
        mcp_config=request.mcp_config,  # Pass through MCP config
    )
    ...
    # Log response summary (always) and content preview (debug)
    output_len = len(response.output) if response.output else 0
    logger.info(
        "execute_template result — agent=%s success=%s output_len=%d session=%s",
        request.agent_name, response.success, output_len, response.session_id,
    )
    if response.output:
        logger.debug(
            "execute_template response.output preview (%d chars): %.500s",
            output_len, response.output,
        )
